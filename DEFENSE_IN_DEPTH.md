...
- [x] `lockdown-repo.sh` applied; `--check` with `--required-checks "tests,zizmor"` and `--allowed-actions "codecov/*"` passes (PRs required on the default branch, merges blocked unless required status checks pass, tag ruleset, immutable releases, fork-PR approval (public repos), read-only workflow tokens, Actions allowlist, secret scanning, Dependabot disabled, private vulnerability reporting (public repos)) — PR #75
